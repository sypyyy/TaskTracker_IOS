...
extension HabitStatisticalCell {
    ...
    private func getDataList(habit: HabitModel) -> [(Int, Color)] {
        ...
        serialQueue.sync {
            targetSeries = masterModel.getTargetSeries(habitID: habit.id)
            stopSeries = masterModel.getStopCheckPoints(habitID: habit.id)
            recordSeries = viewModel.getIntervalRecords(startDate: nil, endDate: nil, habitID: habit.id)
        }
