...
extension HabitController {
    ...
    func createHabit(name: String, detail: String, habitType: HabitType, cycle: String, targetNumber: Int, numberUnit: String,
                            targetHour: Int, targetMinute: Int, setTarget: Bool = true) {
        //let result = habitController(inMemory: true)
        //let viewContext = result.container.viewContext
        let viewContext = self.container.viewContext
        let newHabit = Habit(context: viewContext)
        let id = getUniqueHabitID()
        ...
        case .number:
            ...
            if setTarget {
                ...
                setTargetCheckPoint(habitID: id, date: startDate, numberTarget: target)
            }
        case .time:
            var target = ""
            if setTarget {
                target = (targetHour * 60 + targetMinute).minutesToTime()
                setTargetCheckPoint(habitID: id, date: startDate, timeTarget: target)
            }
        default:
            break
        }
        ...
        if cycle == "Daily" {
            ...
            setStopCheckPoint(habitID: id, date: Date().startOfWeek(), stopPointType: .go)
        } else if cycle == "Monthly" {
            setStopCheckPoint(habitID: id, date: Date().startOfMonth(), stopPointType: .go)
        }
        saveChanges(viewContext: viewContext)
    }
