...
struct StatisticalDetailView: View {
   // @Namespace var animation
    ...
    @Binding var shrinkedRect: CGRect?
                    ...
                    VStack(spacing: 0){
                        ...
                        ScrollView{
                            ...
                            StatDetailContent(habit: habit, enteringChartCycle: enteringChartCycle)
                                //.offset(x: 0, y: appeared ? 0 : screenHeight)
        ...
        var body: some View {
            ...
            Chart(data1) {
                LineMark(
                    ...
                    //series: .value("1111", $0.hoursOfSunshine)
                )
            }
...
struct StatisticalDetailView_Previews: PreviewProvider {
    static var previews: some View {
        GeometryReader { m in
            StatisticalDetailView(isActive: .constant(true), appeared: false, habit: HabitViewModel.shared.getOngoingHabitViewModels()[0], enteringChartCycle: .annually, shrinkedRect: .constant(CGRect(x: 30, y: 150, width: 300, height: 50)), screenWidth:m.size.width, screenHeight: m.size.height)
        }
    }
}
