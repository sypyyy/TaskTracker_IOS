    ...
    var body: some View {
        BottomSheetView{
            ZStack{
                                    ...
                                    if setTarget {
                                        HStack{
                                            if(habitType == "Number") {
                                                ...
                                                Button("\(targetHour) : \(targetMinute)", action: {showTimePicker = true}).buttonHorizontal().padding(.leading)
                                                /*
                ...
                if(showNumberPicker) {
                    numberPickerPopupView(number: $targetNumber, onDoneDidTap: {_ in
                        showNumberPicker = false
                    })
                }
                
                if(showTimePicker) {
                    timePickerPopupView(hour: $targetHour, minute: $targetMinute, onDoneDidTap: { _,_ in
...
struct CreateHabitForm_Previews: PreviewProvider {
    static var previews: some View {
        CreateHabitForm(viewModel: habitTrackerViewModel())
    }
}
